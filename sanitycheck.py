...
def run_sanity_check(test_dir):
    ...
    if not test_functions_for_get:
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"Your test case for GET() does not "
    ...
    if not test_functions_for_post:
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"One or more of your test cases "
                  "for POST() do not seem to be testing the response code.\n")
        ...
        if len(test_functions_for_post) >= 2 and \
            ...
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"You do not seem to have TWO separate "
                  "test cases, one for each possible prediction "
